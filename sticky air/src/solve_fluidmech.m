...
% backup free/no slip boundary commented out
% top boundary
...
dRhodx = (MAT.RhoU(2:end-1,3:end) - MAT.RhoU(2:end-1,1:end-2))./2./NUM.dx;
dRhodz = (MAT.RhoU(3:end,2:end-1) - MAT.RhoU(1:end-2,2:end-1))./2./NUM.dz;
...
aa = -(EtaP1+EtaP2)./NUM.dx^2 - (EtaC1+EtaC2)./NUM.dz^2 - dRhodx.*PHY.gx(2:end-1,2:end-1).*NUM.dt;
...
II = [II; ii(:)]; JJ = [JJ; jj1(:)];   AA = [AA; EtaC1(:)/NUM.dx/NUM.dz- dRhodz(:).*gx(:).*NUM.dt./4];  % W one to the top and left
II = [II; ii(:)]; JJ = [JJ; jj2(:)];   AA = [AA;-EtaC1(:)/NUM.dx/NUM.dz- dRhodz(:).*gx(:).*NUM.dt./4];  % W one to the top and right
II = [II; ii(:)]; JJ = [JJ; jj3(:)];   AA = [AA;-EtaC2(:)/NUM.dx/NUM.dz- dRhodz(:).*gx(:).*NUM.dt./4];  % W one to the bottom and left
II = [II; ii(:)]; JJ = [JJ; jj4(:)];   AA = [AA; EtaC2(:)/NUM.dx/NUM.dz- dRhodz(:).*gx(:).*NUM.dt./4];  % W one to the bottom and right
...
% commented out free/no slip boundary condition
% left boundary
...
%initiate free surface stabilisation
% Density gradients
dRhodx = (MAT.RhoW(2:end-1,3:end) - MAT.RhoW(2:end-1,1:end-2))./2./NUM.dx;
dRhodz = (MAT.RhoW(3:end,2:end-1) - MAT.RhoW(1:end-2,2:end-1))./2./NUM.dz;
% aa = -(EtaP1+EtaP2)./NUM.dz^2 - (EtaC1+EtaC2)./NUM.dx^2;
aa = -(EtaP1+EtaP2)./NUM.dz^2 - (EtaC1+EtaC2)./NUM.dx^2 -dRhodz.*PHY.gz(2:end-1,2:end-1).*NUM.dt;
...
% II = [II; ii(:)]; JJ = [JJ; jj4(:)];   AA = [AA; EtaC2(:)/NUM.dx/NUM.dz];  % W one to the bottom and right
gz = PHY.gz(2:end-1,2:end-1); % free surface stabilisation assembly
II = [II; ii(:)]; JJ = [JJ; jj1(:)];   AA = [AA; EtaC1(:)/NUM.dx/NUM.dz - dRhodx(:).*gz(:).*NUM.dt./4];  % W one to the top and left
II = [II; ii(:)]; JJ = [JJ; jj2(:)];   AA = [AA;-EtaC1(:)/NUM.dx/NUM.dz - dRhodx(:).*gz(:).*NUM.dt./4];  % W one to the bottom and left
II = [II; ii(:)]; JJ = [JJ; jj3(:)];   AA = [AA;-EtaC2(:)/NUM.dx/NUM.dz - dRhodx(:).*gz(:).*NUM.dt./4];  % W one to the top and right
II = [II; ii(:)]; JJ = [JJ; jj4(:)];   AA = [AA; EtaC2(:)/NUM.dx/NUM.dz - dRhodx(:).*gz(:).*NUM.dt./4];  % W one to the bottom and right
