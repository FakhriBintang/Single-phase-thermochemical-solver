...
level_set = 'square'
if RUN.selfgrav
    NUM.RdP = nthroot(NUM.XP.^2 + NUM.ZP.^2,2);     % radial distance from the centre
    switch level_set
        case 'square'
            NUM.PHI = zeros(NUM.nxP,NUM.nzP);
            NUM.RP  = NUM.D/4;
            NUM.PHI = min(-abs(NUM.XP)./NUM.RP + 1,-abs(NUM.ZP)./NUM.RP + 1);
        case 'linear'
            NUM.PHI = zeros(NUM.nxP,NUM.nzP);
            NUM.RP      = NUM.D/25;
...
%             NUM.PHI = cos(pi().*NUM.RdP./NUM.RP./2);
    end
end
...
PHY.gxP = zeros(NUM.nzP,NUM.nxP);
PHY.gzP = zeros(NUM.nzP,NUM.nxP);
PHY.gx  = zeros(NUM.nzU,NUM.nxU) + PHY.gx;
PHY.gz  = zeros(NUM.nzW,NUM.nxW) + PHY.gz;
if RUN.selfgrav; solve_gravity; end

MAT.Rho0(NUM.PHI<=0) = PHY.Rhoa;
% MAT.Eta0(NUM.PHI<=0.2) = PHY.Eta0*100;
MAT.Eta0(NUM.PHI<=0) = PHY.Etaa;
...
switch SOL.Ttype
    ...
    case 'hot bottom'
        SOL.T = zeros(NUM.nzP,NUM.nxP) + SOL.T0;
        SOL.T = SOL.T + SOL.dT.*exp(-(NUM.XP-SOL.xT).^2./SOL.rT.^2 - (NUM.ZP-SOL.zT).^2./SOL.rT.^2 );
        SOL.T(NUM.RdP<NUM.D/6) = SOL.T0+100;
...
% if   RUN.selfgrav; SOL.T = SOL.T.*exp(PHY.aT0*PHY.gP.*(NUM.RP-NUM.RdP)./PHY.Cp0);
...
if RUN.selfgrav; SOL.T(NUM.PHI<=0) = SOL.Ta;
...
if   RUN.selfgrav; SOL.Pt = PHY.Rho0.*PHY.gP.*(NUM.RP-NUM.RdP);      % total pressure
...
% MAT.Hr(NUM.RdP<NUM.D/6)   = PHY.Hr0; % radiogenic heating
Div_PHI  = zeros(NUM.nz ,NUM.nx );              % level set free surface change
